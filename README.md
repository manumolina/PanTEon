...
## Introduction
PanTEon is a framework composed of a free database (PanTEon Database) containing more than 126 thousand automatically curated TEs, from 943 species of animals, plants and fungi and from all TE orders, and a software that can be used for training several models at the same time, compare them in a fair way, infer TE classification from several architectures, and generate TE libraries for any taxonomic or TE classification level.
...
```
Go to the Inpactor2 folder and find the file named "PanTEon_env.yml". Then, install the environment: 
```
