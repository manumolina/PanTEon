...
# Encode tools (use a LabelEncoder per column if they have distinct values)
...
df_resumen.to_excel('Resumen_metricas_ensamble_all.xlsx', index=False)
...
for kingdom in ["animals", "plants", "fungi"]:
    ...
    for col in X_train.columns:
        ...
        le_tool.fit(X_train[col].tolist() + X_test[col].tolist())
        X_train[col] = le_tool.transform(X_train[col])
        X_test[col] = le_tool.transform(X_test[col])  # usar transform, no fit_transform
    ...
    df_resumen = pd.DataFrame(resumen)
    df_resumen.to_excel(f'Resumen_metricas_ensamble_{kingdom}.xlsx', index=False)